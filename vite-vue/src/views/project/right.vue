	...
	<n-layout>
		<div class="project-right">
			<div class="project-header">
				<n-input size="small" placeholder="输入项目名称" v-model="search">
				</n-input>
				<div>
			...
			<div class="project-body">
				...
				<div
					...
					:class="[
						'project-item',
						manage ? 'project-item-manage' : '',
						selectedProjects.includes(item) ? 'project-item-selected' : '',
					]"
...
const filteredProjects = computed(() =>
	projects.value.filter((item) => item.projectName.includes(search.value))
);
...
const openProject = (item: ProjectProps) => {
	(window as any).api.ipcRenderer
		.invoke(CommonChannel.OPEN_NEW_WINDOW, toRaw(item))
		.then(() => {
	...
	dialog.warning({
		...
		onPositiveClick: () => {
			selectedProjects.value.forEach((item) => {
				projectStore.deleteProject(toRaw(item));
			});
			cannelManage();
		},
	});
